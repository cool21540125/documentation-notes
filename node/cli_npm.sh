...
### -------------------------------- CICD 環境使用 npm 安裝套件的議題 --------------------------------
# npm 不會自動偵測 CI 環境, 行為與 yarn & pnpm 不同
#
# 於 CI 系統當中, 建議明確使用:
#    $ npm ci
#
...
### 第 1 招 -- 適合在未上線新專案使用? (林北就是啥都要用最新的就是了)
npm update
# 1. 更新 package-lock.json & node_modules (在 semver 範圍內升級至最新版本)
# 2. 不會異動 package.json 的 semver 範圍設定

### 第 2 招 -- 適合用來在 Development 環境使用? (我要的東西不要給我改版本, 至於間接依賴的東西的版本你自己看著辦)
npm install
# 1. 更新 package-lock.json
# 2. 依照 package-lock.json 安裝套件

### 第 3 招 -- 適合用來做 CI/CD, testing 等等 (叫你安裝啥版本就用啥版本, 不要給我偷偷安裝其他版本)
npm ci
