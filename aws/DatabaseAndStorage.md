...
- Security
  ...
  - 關於 IAM authentication
    - ElastiCache 裡頭, 不支援 _IAM authentication_, 這認證僅支援 API-level Security (delete cache, create cache, ...)
    - if redis
...
- 如果 S3 非常龐大, 又需要做 Query, 可使用 **Redshift Spectrum**
  ...
  - 直接對 S3 query (免 load), 資料不會進入我們的 Nodes, 會在 _Redshift Spectrum_(AWS Service) 查詢完後回傳結果
    - Query -> _Redshift Cluter_ 內的 _Leader Node_
    - _Leader Node_ 分派給 _Compute Nodes_
    - _Compute Nodes_ 再分派給 _Redshift Spectrum_
    - _Redshift Spectrum_ 會對 S3 做資料查詢
...
- Redshift Enhanced VPC Routing
  - 藉由 `COPY Command` && `UNLOAD Command`, 可將 _VPC 內的 data (S3)_ 直接走 private, Copy 到 RedShift
...
- AWS managed extract, transform and load service(ETL)
  ...
  - ETL, Serverless
- 如果在 Athena 使用 `CREATE TABLE`, 則會在 Data Catalog 裡頭建立 metadata
- Serverless
- 家族服務:
  - Glue Data Catalog
...
- 可適用於 Big Data
  - Search / Indexing
- 整合了 _Kinesis Data Firehose_, _AWS IoT_, _CloudWatch Logs_, ...
- Operations
  - 類似 RDS
- Security
  - Cognito
