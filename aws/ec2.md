...
- 申請 EC2 時, 有幾個構面需要優先思考:
  ...
  - Instance Type
    ...
    - Instance Family
      - M5, M6, T2, T3, ...
- (強烈建議) EC2 Termination 以前都做 snapshot 或 CreateImage
- EC2 backup
  - 針對 EBS 做 backup (take EBS snapshot)
  - 針對 EC2 做 Create AMI
    - 其實底層是先建立 AMI, 然後再建立 EBS snapshot (然後關聯到 AMI)
    - 若要移除 AMI, 要先 Deregister AMI, 再來砍 snapshot
